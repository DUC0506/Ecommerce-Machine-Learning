...
import { createUser, getSellers, getUser, getUsers, removeUser, updateUser } from '../../api/user';
...
const SellersAdmin = () => {
  ...
  const fetchUsers = async () => {
    try {
      const {type, message,users} = await getSellers();
  ...
  const handleUpdateUser= async(user1)=>{
    console.log(user1);
    const id=user1._id;
    const {type,message,apartment}=await updateUser(id,user1)
    if (type === 'Error') return message;
    fetchUsers()
  }
  const handleAddUser=async(newUser)=>{
    console.log(newUser);
    const {type,message,user}=await createUser(newUser)
    if (type === 'Error') return message;
    fetchUsers()
  }
  ...
  return (
    <div className='h-full '>
      ...
      <div className="flex justify-between mb-4">
                <h1 className="text-3xl font-medium font-sans">Quản lý nhà bán hàng</h1>
                <button onClick={() => setAddUserModalOpen(true)} className="bg-yellow-400 text-white px-4 py-2 rounded font-sans font-medium">
                Thêm người dùng
                </button>
            </div>
			<div>
        ...
        <AddUserModal isOpen={isAddUserModalOpen} 
