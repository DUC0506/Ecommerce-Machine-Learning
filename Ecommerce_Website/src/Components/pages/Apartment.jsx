...
import { createApartment, getAllApartments, getApartment, removeApartment, updateApartment } from '../../api/apartment';
...
const ApartmentDashboard = () => {
  ...
  const fetchApartments = async () => {
    try {
      ...
      console.log(apartments); // Gọi hàm API để lấy danh sách apartment
      setApartments(apartments); // Cập nhật state apartments với dữ liệu apartment lấy được từ API
    } catch (error) {
  ...
  const handleUpdateApartment= async(apartment1)=>{
    ...
    const {type,message,apartment}=await updateApartment(id,apartment1)
    if (type === 'Error') return message;
    fetchApartments()
  }
  const handleAddApartment=async(newApartment)=>{
    console.log(newApartment);
    const {type,message,apartment}=await createApartment(newApartment)
    if (type === 'Error') return message;
    fetchApartments()
  }
  ...
  return (
    <div className='h-full '>
      ...
      <div className="flex justify-between mb-4">
                <h1 className="text-3xl font-medium font-sans">Quản lý khu chung cư</h1>
                <button onClick={() => setAddApartmentModalOpen(true)} className="bg-yellow-400 text-white px-4 py-2 rounded font-sans font-medium">
                Thêm Apartment
                </button>
            </div>
			<div>
        ...
        {apartments.map(apartment => (
         <div key={apartment.id} className={ isUpdateApartmentModalOpen || isAddApartmentModalOpen 
         ? ` hidden ` :`bg-white rounded-lg shadow-md p-6 mb-2 flex justify-between `}>
          <div>
          <h3 className="text-xl font-medium font-sans mb-2">{apartment.name}</h3>
        ...
        <AddApartmentModal isOpen={isAddApartmentModalOpen} 
