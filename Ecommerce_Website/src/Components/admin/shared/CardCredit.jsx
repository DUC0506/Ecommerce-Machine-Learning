...
export default function CardCredit({ addCreditCard, isClose }) {
  ...
  const handleSubmit = () => {
    if (card.bankNumber === "" || card.bankDate === "" || card.bankCVV === "")
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <form class={`max-w-sm bg-white px-6 py-4 rounded  `}>
        <div className="w-full flex justify-between mb-4">
          <img src={card1} alt="card" className="w-8 mr-2 " />

          <TiDelete className="text-xl" onClick={isClose} />
        </div>
        ...
        <div class="relative">
          <input
            ...
            required
          />
          <div class="absolute inset-y-0 end-0 top-0 flex items-center right-3.5 pointer-events-none">
            <svg
              fill="none"
              class="h-6 text-[#1434CB] dark:text-white"
              viewBox="0 0 36 21"
            >
              <path
                fill="currentColor"
