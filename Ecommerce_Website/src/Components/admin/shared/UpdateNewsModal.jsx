...
export default function UpdateNewsModal({ name, avatar, content, images, timestamp, video, id ,product ,isOpen,onRequestClose ,onSavePost}) {
    ...
    <div className={`absolute z-40 backdrop-blur-sm bg-white/30 w-screen h-screen top-0 bottom-0 right-0 left-0   items-center justify-center  ${   isOpen ? 'flex' : 'hidden'}  `}>
      <div className={`bg-white p-4 mb-4 rounded-lg shadow-md cursor-pointer relative w-1/2  `}
        ...
        <textarea className="mt-2 w-full" value={contentNews} onChange={handleChange}></textarea>
        ...
        <div className="flex justify-end py-2  border-b-2  border-b-inherit">
          <button onClick={()=>handleSavePost()}  className="bg-yellow-500 font-sans font-medium text-white flex items-center  px-4 py-2 mr-1 rounded"><MdSaveAlt   className='text-white text-lg mr-2'/>Lưu </button>
