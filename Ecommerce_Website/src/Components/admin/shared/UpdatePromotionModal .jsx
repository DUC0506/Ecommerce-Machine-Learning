...
const UpdatePromotionModal = ({
  ...
  const handleChange = (e) => {
    ...
    setEditedPromotion((prevApartment) => ({
      ...prevApartment,
      [name]: value,
    }));
  };
  ...
  return (
    ...
    >
      <h2 class="text-2xl  mb-4 font-sans font-medium font-sans">
        Thông tin cơ bản
      </h2>

      <div class="mb-8 bg-white rounded p-4">
        <label for="name" class=" mb-2 flex font-sans font-medium">
          <p className="text-red-500">*</p>Tên chiến dịch quảng cáo{" "}
        </label>
        <input
          type="text"
          id="name"
        ...
        <label for="startDate" class="flex mb-2 font-sans font-medium">
          <p className="text-red-500 ">*</p>Ngày bắt đầu
        </label>
        <input
          type="date"
          id="startDate"
          name="startDate"
          class="w-full border p-2 mb-4 focus:outline-none font-sans  focus:border-yellow-500"
          value={
            new Date(editedPromotion.startDate).toISOString().split("T")[0]
          }
          onChange={handleChange}
        />

        <label for="endDate" class="flex mb-2 font-sans font-medium">
          <p className="text-red-500">*</p>Ngày kết thúc
        </label>
        <input
          type="date"
          id="endDate"
          name="endDate"
          class="w-full border p-2 mb-4 focus:outline-none  focus:border-yellow-500 font-sans"
          value={new Date(editedPromotion.endDate).toISOString().split("T")[0]}
          onChange={handleChange}
        />
      </div>
      <h2 class="text-2xl  mb-4 font-sans font-medium">Ảnh</h2>
      <img
        src={editedPromotion.mainImage}
        alt="promotion-img"
        className="w-32"
      />
      ...
      <div class="flex justify-end">
        <button
          type="button"
          class="bg-yellow-500 text-white px-4 py-2 rounded mr-2 font-sans"
